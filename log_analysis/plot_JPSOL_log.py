...
def get_re_obj(type_num):
    ...
    if(type_num == 0):
        ...
        my_obj = re.compile(r'.*?Total Number Of Unknowns of linear system is (?P<num_rows_A>\d+)'
                            ,re.S)
    ...
    #                         ,re.S)
        ...
        my_obj = re.compile(r'.*?Initial L2 norm of residual: (?P<init_residual>\d+.\d+)'
                            ,re.S)
    elif(type_num == 13):
        my_obj = re.compile(r'.*?Converged after (?P<iter>\d+) iters with residual (?P<final_residual>.*?)\n'
                            ,re.S)
    elif(type_num == 20):
